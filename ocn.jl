...
for i=1:nrow*ncol, j=1:nrow*ncol
    if (div(i, ncol) == div(j, ncol)) && (i != j) && (j == i + 1)
        W[i, j] = 1
    end
...
### DOESNT WORK down_nodes = [(1:nrow*ncol)[W[i, :] .== 1] for i=1:nrow*ncol]
for i in outlets
    insert!(down_nodes, i, -1)
end
A = compute_A(W, alpha)
H = compute_H(A, gamma)
energy = zeros(n_iter)
...
for i = 1:n_iter
    valid_rewirings = zeros(nrow*ncol, nrow*ncol)
    ...
    while true
        while true
            ...
            if valid_rewirings[node, down_node_new] == 1
                break
            elseif is_invertible(I - transpose(W_new)) && !is_cross_flow(down_nodes, node, down_node_new, neighbors[node], X_idx, Y_idx)
    ...
    A = A_new
    W = copy(W_new)
    down_nodes = [(1:nrow*ncol)[row == 1] for row in W]
    for i in outlets
        insert!(down_nodes, i, -1)
    end
end
